        ...
        /*===| 根据档位限制旋转速度，遥控器控制状态不限制 |===*/
			switch(RoboControl_Struct.Chassis_Speed_Level)
			{
				...
				case 2:
					...
					break;
				case 3:
					RoboControl_Struct.Robo_Target_Wz = RoboControl_Struct.Robo_Target_Wz*(9.00f/12.0f);
					break;
				case 4:
					RoboControl_Struct.Robo_Target_Wz = RoboControl_Struct.Robo_Target_Wz*(10.5f/12.0f);
					break; 
				case 5:
    ...
    float Dt = DWT_GetDeltaT(&Remote_DWT_Count);
		...
		else if(Remote_Control_Struct.RC_Side < -0.7f)
		{
            ...
            if      (Remote_Control_Struct.RC_Right_X > 0.8f && Remote_Control_Last_Struct.RC_Right_X < 0.8f)
            {
				...
				Chassis_Control_Struct.UpStep_Dect_Flag = 0;//机器人上台阶磕碰检测标志位
