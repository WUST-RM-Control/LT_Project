...
//用于检测发射出的弹丸时间戳，给小电脑自瞄用的
//弹丸加速完成的时间戳
uint32_t Tim_fire_cmd_apply_mcu_us;
//发射出去的弹丸数量
uint32_t fire_bullet_count=0;
//在一个加速弹丸周期内摩擦轮电机电流到达波峰
uint8_t Shoot_Fric_Output_Max_Flag=0;

void Fire_Tim(void)
{
    if(fire_bullet_count!=Aim_Send_Struct.bullet_count && ((Shoot_Control_Struct.Shoot_State == Shoot_State_Ready)||(Shoot_Control_Struct.Shoot_State == Shoot_State_Continue)))
    {
        if(fabs(Shoot_Control_Struct.Fric_First_Left_Speed_PID_Struct.Output-Shoot_Control_Struct.Fric_First_Right_Speed_PID_Struct.Output)>6500.0f)
        {
            Shoot_Fric_Output_Max_Flag = 1;
        }
    }
    if(Shoot_Fric_Output_Max_Flag&&(fabs(Shoot_Control_Struct.Fric_First_Left_Speed_PID_Struct.Error)<fabs(Shoot_Control_Struct.Fric_First_Left_Speed_PID_Struct.Error_Last)))
    {
        fire_bullet_count = Aim_Send_Struct.bullet_count;
        Tim_fire_cmd_apply_mcu_us = DWT->CYCCNT/170.0f;
        Shoot_Fric_Output_Max_Flag = 0;
    }
}
